...
public class WaysToReachNthStair {
	...
	public static void main(String[] args) {
		int steps = getNextInt();
		int[] answers = new int[steps + 1];
		answers[0] = 0; // if there are no steps to climb
		answers[1] = 1; // by climbing one step
		answers[2] = 2; // either by climbing one step at a time or two steps at once
		for(int i=3; i<=steps; ++i){
			//there are only two possible ways to reach a particular step.
			//either by taking one step from the previous step
			//or by taking two steps at once from the step before the previous step
			answers[i] = answers[i-1] + answers[i-2];
		}
		System.out.println(answers[steps]);
	}
}

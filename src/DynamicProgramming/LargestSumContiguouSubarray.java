...
public class LargestSumContiguouSubarray {
	...
	public static int solve(int[] a){
		int maxSoFar = a[0]; //dont initialize with Integer.MIN_VALUE because if the elements are negative, it goes beyond the negative min range and starts from positive range in a circular fashion. so effectively, Integer.MIN_VALUE -2 = Integer.MAX_VALUE -1
		int currentMax = a[0];
		//the idea is that at every element, we can either consider that element and see backwards what is the sum containing that element, or if its best to take just that element, in case of negative sum for elements before
		//the other way is to no consider this element and going with greatest sum so far
